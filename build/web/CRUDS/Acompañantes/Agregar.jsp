...
<html>
    ...
    <body>
        <%
            ...
            PreparedStatement ps0,ps, ps1, ps2, ps3, ps4, ps5, ps6, ps7, ps8, ps9, ps10, ps11, ps12, ps13, ps14, ps15, ps16, ps17, ps18, ps19, ps20, ps21, ps22, ps23, ps24, ps25, ps26, ps27, ps28, ps29, ps30, ps31, ps32;
            ResultSet rs0,rs, rs1, rs2, rs3, rs4, rs5, rs6, rs7, rs8, rs9, rs10, rs11, rs12, rs13, rs14, rs15, rs16, rs17, rs18, rs19, rs20, rs21, rs22, rs23, rs24, rs25, rs26, rs27, rs28, rs29, rs30, rs31, rs32;
            ps = con.prepareStatement("select * from pasaje");
            rs = ps.executeQuery();
            ps0 = con.prepareStatement("select * from cliente");
        ...
        <div class="container">
            ...
            <form action="" method="post" class="form-control" style="width:500px;height:550px">
                ...
                <select  name="txtCli" required class="form-control">
                    <%while(rs0.next()){%>
                    <option value="<%=rs0.getInt("idCliente")%>"><%=rs0.getString("Nombres")%></option>
                    <%}%>
                    </select>
                Nombres:
                ...
                Pasaje :
                <select type="text" name="txtPasa" required class="form-control">
                    <% while (rs.next()) {%>
                    <option value="<%=rs.getInt("idPasaje")%>"><%=rs.getInt("idPasaje")%></option>
                    <%}%>
                </select>
                Asiento:
...
<%
    ...
    String cliente,nombre, paterno, materno, fecha, dni, pasaje, asiento;
    cliente = request.getParameter("txtCli");
    ...
    if (cliente != null && nombre != null && paterno != null && materno != null && fecha != null && dni != null && pasaje != null) {
        cs = con.prepareCall("{CALL SP_PERSONA_ASIENTO('" + nombre + "','" + paterno + "','" + materno + "','" + fecha + "','" + dni + "','" + pasaje + "','" + asiento + "','" + cliente + "')}");
        cs.executeUpdate();
