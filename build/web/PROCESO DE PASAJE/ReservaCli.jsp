...
<html>
    <head>
        ...
        <link href="../css/StyleImg.css?v=2.0" rel="stylesheet" type="text/css"/>
    ...
    <body>
        <%
            ...
            ps = con.prepareStatement("select idPasaje,CONCAT(co.Nombre_ciudad,'-',cd.Nombre_ciudad) AS Ruta from pasaje p inner join ciudad co on p.idCiudad_Origen=co.idCiudad inner join ciudad cd on p.idCiudad_Destino=cd.idCiudad where idPasaje=" + id);
            rs = ps.executeQuery();
        ...
        <div class="container">
            ...
            <hr>
            <div class="costado">
                <form action="" method="post" class="form-control" style="width:500px;height:850px">
                    Id Pasaje:
                    <%while (rs.next()) {%>
                    <input type="text" name="txtPasa" readonly required class="form-control" value="<%=rs.getInt("idPasaje")%>"/>
                    Ruta:
                    ...
                    Apellido Paterno:
                    <input type="text" name="txtAp" required class="form-control"/>
                    Apellido Materno:
                    <input type="text" name="txtAm" required class="form-control"/>
                    Fecha Nacimiento:
                    <input type="date" name="txtFn" required class="form-control"/>
                    DNI:
                    <input type="text" name="txtDNI" minlength="8" maxlength="8" required class="form-control"/>
                    Email:
                    <input type="email" name="txtEmail" required class="form-control"/>
                    Celular:
                    <input type="text" name="txtCel" minlength="9" maxlength="9" required class="form-control"/>
                    Nacionalidad:
                    <input type="text" name="txtNac" required class="form-control"/>
                    Asiento:
                    <input type="text" id="asiento" required readonly name="txtAsi" class="form-control"/>
                    Estado:
                    <input type="text" class="form-control" id="estado"  name="txtEst" data-readonly required pattern="Disponible"/>
                    <br>
                    <input type="submit" value="Guardar" class="btn btn-primary">
                    <%}%>
                    <a href="ListarPasajes.jsp">Regresar</a>
                </form>
...
<%
    ...
    String nombre, paterno, materno, fecha, dni, email, celular, nacionalidad, asiento, pasaje;
    ...
    nacionalidad = request.getParameter("txtNac");
    asiento = request.getParameter("txtAsi");
    pasaje = request.getParameter("txtPasa");
    if (nombre != null && paterno != null && materno != null && fecha != null && dni != null && email != null && celular != null && nacionalidad != null && asiento != null && pasaje != null) {
        cs = con.prepareCall("{CALL SP_CLIENTE_ASIENTO('" + nombre + "','" + paterno + "','" + materno + "','" + fecha + "','" + dni + "','" + email + "','" + celular + "','" + nacionalidad + "','" + asiento + "')}");
        cs.executeUpdate();
        response.sendRedirect("AgregacionReserva.jsp?idAs=" + asiento + "&idPasa=" + pasaje);
    }
%>